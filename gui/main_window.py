...
# Step импорттары (кеш жүктеу үшін ретке келтірілген)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.state = AppState()
        self.setWindowTitle("XLSX → DOCX Flow")
        self.setMinimumSize(900, 600)

        # OUTER CONTAINER
        self.outer_container = QWidget()
        self.outer_container.setObjectName("outer_container")
        container_layout = QVBoxLayout(self.outer_container)
        ...
        # finalize layout
        container_layout.addWidget(self.central_panel)
